...
def signup():
	if request.method == 'POST':
		...
		return render_template("signup.html", isLoggedIn=isLoggedIn)
...
def display_course_info(id):
    ...
    return render_template("courseinfo.html", courses = courses[0], isLoggedIn=isLoggedIn)
...
def display_recommendations():
	...
	for i in range(1,9):
		...
		for nameCourse in user['courses'][str(i)]:
			aCourse = db_courses.find_one({'course' : nameCourse})
			courses_list[str(i)].append(aCourse)
	return render_template("recommendations.html", courses = courses_list, needToTake = needToTake, isLoggedIn=isLoggedIn)
